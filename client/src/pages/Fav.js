...
const Fav = () => {
  ...
  const getUserLikedBlogs = async () => {
    try {
      ...
      const { data } = await axios.get(`https://blogapp-9vhq.onrender.com/api/v1/blog/user-blog/${id}`);
  ...
  const fetchLikedBlogsDetails = async () => {
    try {
        const detailedBlogs = await Promise.all(
          ...
          const { data } = await axios.get(`https://blogapp-9vhq.onrender.com/api/v1/blog/get-blog/${likedBlogId}`);
        //   console.log(data);
